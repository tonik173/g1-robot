...
int main() 
{
    try {
        ...
        // First, a domain participant is needed. Create one on the default domain.
        ...
        // Poll until a message has been read. It isn't really recommended to do this kind wait in a polling loop.
        // It's done here just to illustrate the easiest way to get data. Please take a look at Listeners and WaitSets 
        // for much better solutions, albeit somewhat more elaborate ones.
        std::cout << "=== [Subscriber] Wait for message." << std::endl;
        bool poll = true;
        while (poll) {
            // For this example, the reader will return a set of messages (aka Samples). There are other ways of 
            // getting samples from reader.  See the various read() and take() functions that are present.
            ...
            if (samples.length() > 0) {
                ...
                for (sample_iter = samples.begin(); sample_iter < samples.end(); ++sample_iter) {
                    ...
                    // Sometimes a sample is read, only to indicate a data state change (which can be found in the info). 
                    // If that's the case, only the key value of the sample is set. The other data parts are not.
                    if (info.valid()) {
