...
static void print_stats(times &to_print)
{
  ...
  std::cout 
    << std::setw(column_width) << to_print[to_print.size()/2] << std::setw(column_width) << to_print.front() 
    << std::setw(column_width) << to_print[static_cast<size_t>(static_cast<double>(to_print.size())*0.99)] 
    << std::setw(column_width) << to_print.back() << std::flush;
  to_print.clear();
}
...
// main data processing function. This will take the received samples, store the time difference between sending the previous message and
// receiving the current and send the return message with the current timestamp
...
static bool parse_args(int argc, char *argv[])
{
  ...
  std::cout << "# payloadSize: " << payloadSize << " | numSamples: " << numSamples << " | timeOut: " << timeOut  
            << " | using " << (use_listener ? "listener" : "waitset") << " method\n" << std::flush;
  return true;
}
...
int main (int argc, char *argv[])
{
  try {
    ...
    dds::sub::DataReader<RoundTripModule::DataType>
      reader(subscriber, topic, dds::sub::qos::DataReaderQos(), list,
